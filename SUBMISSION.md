...
Iolaus targets one missing boundary in that loop:
...
Iolaus changes one rule:

> **No receipt → no trusted transition.**

Every action has a small contract: verified claims it requires, claims it may produce, and the verifier(s) authorized to certify it. Execution results are written to HydraDB only as `SUCCEEDED_UNVERIFIED` observations. An independent verifier then checks the declared postcondition. Iolaus signs and stores the verifier evidence. Only a PASS receipt can create a trusted `HBClaim`, and only those claims can enable downstream actions.

The live demo deliberately uses a lying tool that returns `success=true` without creating the requested CRM record. Iolaus rejects it, creates no trusted claim, and blocks the downstream welcome email. The same action succeeds after a deterministic readback proves the record exists.
...
The narrow benchmark adds one metric to Hydra's existing routing/plan metrics: **false-success commit rate**. A naive baseline trusts `success=true`; Iolaus requires the postcondition. The acceptance gate is zero false trusted commits across adversarial semantic-failure cases.

The closest research, such as ToolGate, already shows why pre/postcondition-gated tool state matters. Iolaus's contribution is making that verifier-gated transition a durable, traversable HydraDB primitive at the exact point where agent execution becomes shared state and future learning signal.
