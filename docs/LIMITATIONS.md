...
Iolaus v0.1 is intentionally an atomic primitive, not a complete agent platform.

- **Verifier quality is external.** Iolaus enforces that a verifier exists and is authorized; it does not magically make a poor verifier correct.
...
- **Demo world is deterministic.** SQLite represents an external target system solely so silent failures have auditable ground truth. It is not used as Iolaus's graph store.
